...
unsigned int getLength(char character[]) {
    unsigned int i = 0;
    while(1) {
        if(character[i] == '\n')
        {
...
void delayOneMS(unsigned int n) {
    while(n--) {
        TMR1H = 252;
        TMR1L = 24;
        TMR1IF = 0;
...
void main(void) {
    ...
    const char info[] = {0x03, 'T', 'V', 'T', '1', '5', 'A', '-', '1', '9', '4', '3', '6', '4', '8' ,'1', '\n'};
    const char end_first[] = {'T', 'h', 'i', ' ', 'k', 0x00, 't', ' ', 't', 'h', 0x01, 'c', ' ', 'm', 0x02, 'n', '\n'};
    const char end_second[] = {'K', 'T', ' ', 'v', 'i', ' ', 'x', 0x03, ' ', 'l', 0x04, '\n'};
    const char special_name_info[]
    = {
        13, 18, 8, 20, 8, 20, 30, 0,
    ...
    while(1) {
        if(!RB0) {
            while(!RB0);
            lcd_init();
            insertCharacter(special_name_info);
            lcd_putc('\f');
            lcd_gotoxy(0, 0);
            length = getLength(your_name);
            for(i = 0; i < length; i++) {
            ...
            while(!RB1);
            lcd_init();
            insertCharacter(special_end);
            lcd_gotoxy(0, 0);
            length = getLength(end_first);
            for(i = 0; i < length; i++) {
                lcd_putc(end_first[i]);
            }
            length = getLength(end_second);
            lcd_gotoxy(2, 1);
            for(i = 0; i < length; i++) {
                lcd_putc(end_second[i]);
            }
            unsigned int delays = 2;
            delayOneMS(2000);
            lcd_putc('\f');
        }
    }
